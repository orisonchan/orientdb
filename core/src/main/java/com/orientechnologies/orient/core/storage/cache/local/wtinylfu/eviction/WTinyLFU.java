...
public final class WTinyLFU {
  ...
  public void onPinning(OCacheEntry cacheEntry) {
    ...
    while (protection.size() > maxProtectedSize) {
      final OCacheEntry victim = protection.poll();
      probation.moveToTheTail(victim);
    }

    while (probation.size() > maxProbationarySize) {
      final OCacheEntry victim = probation.poll();
      assert victim != null;
