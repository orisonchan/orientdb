 ...
 * @since 12.03.13
 */
public final class OLocalHashTable<K, V> extends ODurableComponent {
  ...
  public void create(final OBinarySerializer<K> keySerializer, final OBinarySerializer<V> valueSerializer, final OType[] keyTypes,
    ...
    try {
      ...
      try {
        ...
        fileStateId = addFile(getName() + metadataConfigurationFileExtension, atomicOperation);

        directory.create(atomicOperation);

        final OCacheEntry hashStateEntry = addPage(fileStateId, false);
        pinPage(hashStateEntry);

        OHashIndexFileLevelMetadataPage page = null;
        try {
          page = new OHashIndexFileLevelMetadataPage(hashStateEntry);
          page.init();
        ...
        fileId = addFile(fileName, atomicOperation);
        ...
        if (nullKeyIsSupported) {
          nullBucketFileId = addFile(getName() + nullBucketFileExtension, atomicOperation);
        }
      } finally {
    ...
    try {
      ...
      try {
        ...
        if (key == null) {
          ...
          try {
            final OHashIndexBucket<K, V> bucket = new OHashIndexBucket<>(cacheEntry);

            final OHashIndexBucket.Entry<K, V> entry = bucket.find(key, hashCode, keySerializer, valueSerializer, encryption);
            if (entry == null) {
              return null;
            }
  ...
  public boolean validatedPut(final K key, final V value, final OIndexEngine.Validator<K, V> validator) throws IOException {
  ...
  public V remove(K key) throws IOException {
    ...
    try {
      ...
      try {
        ...
        if (key != null) {
          key = keySerializer.preprocess(key, (Object[]) keyTypes);
          final byte[] rawKey = keySerializer.serializeNativeAsWhole(key, (Object[]) keyTypes);
          ...
          try {
            bucket = new OHashIndexBucket<>(cacheEntry);
            final int positionIndex = bucket.getIndex(hashCode, key, keySerializer, encryption);
            found = positionIndex >= 0;

            if (found) {
              final byte[] rawValue = bucket.getRawValue(positionIndex, encryption != null, keySerializer, valueSerializer);
              removed = valueSerializer.deserializeNativeObject(rawValue, 0);
              bucket.deleteEntry(positionIndex, rawKey.length, rawValue.length);
              sizeDiff--;
            } else {
              removed = null;
            }
  ...
  public OHashIndexBucket.Entry<K, V>[] higherEntries(final K key) {
    ...
    try {
      ...
      try {
        ...
        try {
          OHashIndexBucket<K, V> bucket = new OHashIndexBucket<>(cacheEntry);

          while (bucket.size() == 0 || comparator.compare(bucket.getKey(bucket.size() - 1, keySerializer, encryption), key1) <= 0) {
          ...
          final int index = bucket.getIndex(hashCode, key1, keySerializer, encryption);
    ...
    try {
      ...
      try {
        if (isFileExists(name + metadataConfigurationFileExtension)) {
          fileStateId = openFile(name + metadataConfigurationFileExtension);
          deleteFile(fileStateId, atomicOperation);
        }
        ...
        if (isFileExists(name + nullBucketFileExtension)) {
          ...
          deleteFile(nullBucketId, atomicOperation);
        }
  ...
  private OHashIndexBucket.Entry<K, V>[] convertBucketToEntries(final OHashIndexBucket<K, V> bucket, final int startIndex,
    ...
    final Iterator<OHashIndexBucket.Entry<K, V>> iterator = bucket.iterator(startIndex, keySerializer, valueSerializer, encryption);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          final int index = bucket.getIndex(hashCode, key, keySerializer, encryption);
          final int startIndex;
          if (index >= 0) {
            startIndex = index;
          } else {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          while (bucket.size() == 0) {
            final BucketPath prevBucketPath = prevBucketToFind(bucketPath, bucket.getDepth(), atomicOperation);
  ...
  public OHashIndexBucket.Entry<K, V>[] lowerEntries(K key) {
    ...
    try {
      ...
      try {
        ...
        try {
          OHashIndexBucket<K, V> bucket = new OHashIndexBucket<>(cacheEntry);
          while (bucket.size() == 0 || comparator.compare(bucket.getKey(0, keySerializer, encryption), key) >= 0) {
            final BucketPath prevBucketPath = prevBucketToFind(bucketPath, bucket.getDepth(), atomicOperation);
            if (prevBucketPath == null) {
              //noinspection unchecked
              return OCommonConst.EMPTY_BUCKET_ENTRY_ARRAY;
  ...
  public long size() {
    ...
    try {
      ...
      try {
        ...
        try {
          final OHashIndexFileLevelMetadataPage metadataPage = new OHashIndexFileLevelMetadataPage(hashStateEntry);
  ...
  private boolean put(K key, final V value, final OIndexEngine.Validator<K, V> validator) throws IOException {
    boolean rollback = false;
    final OAtomicOperation atomicOperation = startAtomicOperation(true);
    try {
      acquireExclusiveLock();
      try {
        ...
        if (key == null) {
          ...
          rawKey = keySerializer.serializeNativeAsWhole(key, (Object[]) keyTypes);
        } else {
  ...
  private boolean doPut(final K key, V value, final byte[] rawKey, byte[] rawValue, final OIndexEngine.Validator<K, V> validator,
    ...
    if (key == null) {
      ...
      try {
        ...
        if (index > -1) {
          rawOldValue = bucket.getRawValue(index, encryption != null, keySerializer, valueSerializer);
        } else {
          rawOldValue = null;
        }
        ...
        if (index > -1) {
          assert rawOldValue != null;
          if (rawOldValue.length == rawValue.length) {
            bucket.updateEntry(index, rawValue, rawOldValue.length, rawKey.length);
            changeSize(sizeDiff, atomicOperation);
            return true;
          }

          bucket.deleteEntry(index, rawKey.length, rawOldValue.length);
          sizeDiff--;
        }

        int insertionIndex = bucket.binarySearch(key, hashCode, keySerializer, encryption);
        if (insertionIndex >= 0) {
          throw new IllegalArgumentException("Given value is present in bucket.");
        }

        insertionIndex = -insertionIndex - 1;
        if (bucket.addEntry(insertionIndex, hashCode, rawKey, rawValue)) {
          sizeDiff++;
      ...
      doPut(key, value, rawKey, rawValue, null /* already validated */, atomicOperation);
      return true;
    }
  }
  ...
  private static boolean assertParentNodeStartIndex(final BucketPath bucketPath, final long[] parentNode,
  ...
  private void addNewLevelNode(final BucketPath bucketPath, final int nodeIndex, final long newBucketPointer,
  ...
  private void splitBucketContent(final OHashIndexBucket<K, V> bucket, final OHashIndexBucket<K, V> newBucket,
    ...
    final List<OHashIndexBucket.RawEntry> entries = new ArrayList<>(bucket.size());
    final Iterator<OHashIndexBucket.RawEntry> iterator = bucket.rawIterator(keySerializer, valueSerializer, encryption);
    while (iterator.hasNext()) {
      final OHashIndexBucket.RawEntry entry = iterator.next();
      entries.add(entry);
    }
    ...
    for (final OHashIndexBucket.RawEntry entry : entries) {
      if (((entry.hashCode >>> (HASH_CODE_SIZE - newBucketDepth)) & 1) == 0) {
        bucket.appendEntry(entry.entry, entry.keySize, entry.valueSize);
      } else {
        newBucket.appendEntry(entry.entry, entry.keySize, entry.valueSize);
      }
    }
  ...
  private BucketSplitResult splitBucket(final OHashIndexBucket<K, V> bucket, final long pageIndex,
      final OAtomicOperation atomicOperation) throws IOException {
    ...
    final OCacheEntry newBucketCacheEntry = addPage(fileId, false);

    OHashIndexBucket<K, V> newBucket = null;
    try {
      newBucket = new OHashIndexBucket<>(newBucketCacheEntry);
  ...
  private void initHashTreeState(final OAtomicOperation atomicOperation) throws IOException {
    ...
    for (long pageIndex = 0; pageIndex < MAX_LEVEL_SIZE; pageIndex++) {
      ...
      try {
        emptyBucket = new OHashIndexBucket<>(cacheEntry);
    ...
    final OCacheEntry hashStateEntry = loadPageForWrite(fileStateId, hashStateEntryIndex, true);
    try {
      metadataPage = new OHashIndexFileLevelMetadataPage(hashStateEntry);
  ...
  private BucketPath getBucket(final long hashCode, final OAtomicOperation atomicOperation) throws IOException {
  ...
  static final class NodeSplitResult {
    final long[]  newNode;
  ...
  static final class KeyHashCodeComparator<K2> implements Comparator<K2> {
