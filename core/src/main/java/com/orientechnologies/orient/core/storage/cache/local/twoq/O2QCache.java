...
public final class O2QCache implements OReadCache {
  ...
  public O2QCache(final long readCacheMaxMemory, final int pageSize, final boolean checkMinSize, final int percentOfPinnedPages,
    ...
    try {
      ...
      this.filePages = new ConcurrentHashMap<>(1024);
  ...
  public final long addFile(final String fileName, final OWriteCache writeCache) throws IOException {
    ...
    try {
      ...
      final Set<Long> oldPages = filePages.put(fileId, Collections.newSetFromMap(new ConcurrentHashMap<>(1024)));
  ...
  public final long addFile(final String fileName, long fileId, final OWriteCache writeCache) throws IOException {
    ...
    try {
      ...
      final Set<Long> oldPages = filePages.put(fid, Collections.newSetFromMap(new ConcurrentHashMap<>(1024)));
      assert oldPages == null || oldPages.isEmpty();
...
//      removeColdestPagesIfNeeded();
  ...
  private UpdateCacheResult doLoad(final long fileId, final long pageIndex, final boolean checkPinnedPages,
      final boolean addNewPages, final boolean initNewPage, final OWriteCache writeCache, final int pageCount,
      final boolean verifyChecksums, final OModifiableBoolean cacheHit) throws IOException {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (cacheEntry == null) {
            final UpdateCacheResult cacheResult = updateCache(fileId, pageIndex, addNewPages, initNewPage, writeCache, pageCount,
  ...
  public final OCacheEntry allocateNewPage(long fileId, final OWriteCache writeCache, final boolean verifyChecksums,
      final OLogSequenceNumber startLSN, final boolean initPage) throws IOException {
    fileId = OAbstractWriteCache.checkFileIdCompatibility(writeCache.getId(), fileId);
  ...
  public final void storeCacheState(final OWriteCache writeCache) {
    ...
    try {
      ...
      throw OException.wrapException(new OStorageException("Cannot store state of cache for storage"), e);
    } finally {
      cacheLock.releaseWriteLock();
    }
  }
  ...
  private UpdateCacheResult entryIsAbsentInQueues(final long fileId, final long pageIndex, final OCachePointer dataPointer) {
    ...
    if (pages == null) {
      pages = Collections.newSetFromMap(new ConcurrentHashMap<>(1024));
  ...
  private UpdateCacheResult updateCache(final long fileId, final long pageIndex, final boolean addNewPages,
      final boolean initNewPage, final OWriteCache writeCache, final int pageCount, final OModifiableBoolean cacheHit,
